...
export default function ContactUs6({ heading, caption, contactDetails }) {
  ...
  const mapContainer = useMemo(() => {
    return (
      <MapContainer key={time} center={[33.2284, -96.8029]} zoom={10} style={{ height: '100%' }}>
        <TileLayer
        ...
        <Grid container spacing={1.5} direction={{ xs: 'column-reverse', md: 'row' }}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Stack sx={{ gap: 1.5 }}>
              ...
              <GraphicsCard sx={{ p: { xs: 3, md: 5 }, backgroundColor: 'grey.contact' }}>
                <Grid container spacing={{ xs: 3, md: 4.25 }}>
                  ...
                  {contactDetails.time && (
                    <Grid size={{ xs: 12, sm: 6, md: 12 }}>
                      ...
                      {Object.entries(contactDetails.time[0]).map(([day, hours]) => (
                        <div key={day}>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <GraphicsCard sx={{ height: 1, backgroundColor: 'grey.contact' }}>
